...
vanlilla_color = '#014c45'
slimclr_color = '#f36162'
...
def plot_lines(json_file=None, extra_json_file=None, filename=None, title=None, title_fontsize=16,
	...
	# ax.plot(x1, y1, label=r'SlimCLR-MoCov2, vanlilla', color='#fc8d62', linewidth=2.5)
	# ax.plot(x2, y2, label=r'SlimCLR-MoCov2 (Ours)', color='#66c2a5', linewidth=2.5)

	ax.plot(x1, y1, label=r'SlimCLR-MoCov2, vanlilla + slow start', color=vanlilla_color, linewidth=2.5)
	ax.plot(x2, y2, label=r'SlimCLR-MoCov2, distillation + reweight', color=slimclr_color, linewidth=2.5)
...
def plot_pca_directions(model_dir, filename, prefix='fc_grad_layer2', ticks=100, overwrite=False):
	if "300" in model_dir:
		color = vanlilla_color
		title = "SlimCLR-MoCov2, vanlilla baseline + slow start"
	else:
		color = slimclr_color
		title = "SlimCLR-MoCov2, distillation + loss reweight"	
	ylims = [-0.35, 0.35]
	zlims = [-0.35, 0.35]
	tick_font = 8
	label_font = 12
...
if __name__ == "__main__":
	...
	# # code for Figure.(4b&c) in the Paper
	# plot directions graph
	overwrite = int(sys.argv[1]) if len(sys.argv) > 1 else False
	model_dir = os.path.join(abs_path, "moco_ImageNet_300")
	filename = "eigenvector_300.pdf"
