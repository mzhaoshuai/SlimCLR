        ...
        if self.bn_eval:
            for m in self.modules():
                if isinstance(m, nn.BatchNorm2d) or isinstance(m, SwitchableBatchNorm2d):
                    m.eval()
                    if self.bn_frozen:
                        # originally, slimmable networks remove this condition
...
def name_remap(x):
    map_dict = {
            'features.0.': 'layer1.0.', 'features.1.': 'layer1.1.', 'features.2.': 'layer1.2.',
            'features.3.': 'layer2.0.', 'features.4.': 'layer2.1.', 'features.5.': 'layer2.2.', 'features.6.': 'layer2.3.',
            'features.7.': 'layer3.0.', 'features.8.': 'layer3.1.', 'features.9.': 'layer3.2.', 'features.10.': 'layer3.3.',
            'features.11.': 'layer3.4.', 'features.12.': 'layer3.5.',
            'features.13.': 'layer4.0.', 'features.14.': 'layer4.1.', 'features.15.': 'layer4.2.',   
            }
    for k, v in map_dict.items():
        if k in x: x = x.replace(k, v)

    x = x.replace("shortcut", "downsample")
    # reverse bn orders
    if "bn.0" in x: 
        return x.replace("bn.0", "bn.3")
    if "bn.1" in x:
        return x.replace("bn.1", "bn.2")
    if "bn.2" in x:
        return x.replace("bn.2", "bn.1")
    if "bn.3" in x:
        return x.replace("bn.3", "bn.0")
    return x


def load_ssl_pretrained(model, pretrained):
    """load checkpoint pre-trained with MoCov2 or MoCov3"""
    ...
    for k in list(state_dict.keys()):
        ...
        # remove prefix
        if 'mocov2' in pretrained:
            if k.startswith('module.encoder_q') and not k.startswith('module.encoder_q.fc'):
                key = name_remap(k[len("module.encoder_q."):])
                state_dict[key] = state_dict[k]

        elif 'mocov3' in pretrained:
...
def load_checkpoint(m, pretrained, strict=False, logger=None):
    ...
    print('[Model]: load checkpoint from {}'.format(pretrained))

    if "moco" in pretrained:
        load_ssl_pretrained(m, pretrained)
...
if __name__ == "__main__":
    FLAGS.pretrained = "/home/shuzhao/Data/unified_lv_env/mocov2_ckpt_e800_num_119.pth.tar"
    model = ResNet(depth=50)
    model.init_weights(pretrained=FLAGS.pretrained)
