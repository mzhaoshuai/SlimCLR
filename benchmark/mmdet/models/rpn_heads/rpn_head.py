...
class RPNHead(nn.Module):
    ...
    def _get_proposals_single(self, rpn_cls_scores, rpn_bbox_preds,
        ...
        for idx in range(len(rpn_cls_scores)):
            ...
            proposals_w_score = torch.cat([proposals, scores.unsqueeze(-1)], dim=-1)
            # proposals = torch.cat([proposals, scores.unsqueeze(-1)], dim=-1)
            # nms_keep = nms(proposals, cfg.nms_thr)[:cfg.nms_post]
            nms_keep = torchvision.ops.nms(proposals, scores, cfg.nms_thr)[:cfg.nms_post]

            # proposals = proposals[nms_keep, :]
            proposals = proposals_w_score[nms_keep, :]
            mlvl_proposals.append(proposals)
        proposals = torch.cat(mlvl_proposals, 0)

        if cfg.nms_across_levels:
            # nms_keep = nms(proposals, cfg.nms_thr)[:cfg.max_num]
            nms_keep = torchvision.ops.nms(proposals[..., 0:4], proposals[..., 4], cfg.nms_thr)[:cfg.nms_post]
            proposals = proposals[nms_keep, :]
        else:
            scores = proposals[:, 4]
            _, order = scores.sort(0, descending=True)
